 ...
 * <p>
 * 提供了外部运维管理服务的接口（可通过 Web UI 触发下述方法）
 *
 * @author nkorange
 */
@RestController
...
public class ServiceController {
  ...
  /**
   * 根据正则表达式查询服务（根据是否指定 responsibleOnly 返回当前节点负责的或者全部符合条件的）
   * 
   * @param request
   * @return
   */
  ...
  /**
   * 响应其它 nacos 节点发来的更新服务校验和的请求。当本地保存的与请求中不一致，用请求中的校验和更新本地数据。
   */
      ...
      for (Map.Entry<String, String> entry : checksums.serviceName2Checksum.entrySet()) {
        ...
        if (!checksum.equals(service.getChecksum())) {
          if (Loggers.SRV_LOG.isDebugEnabled()) {
            Loggers.SRV_LOG.debug("checksum of {} is not consistent, remote: {}, checksum: {}, local: {}", serviceName,
                serverIP, checksum, service.getChecksum());
          }
