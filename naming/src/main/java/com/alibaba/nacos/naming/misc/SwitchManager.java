...
public class SwitchManager implements RecordListener<SwitchDomain> {
  ...
  /**
   * 根据 entry 取值,使用 value 更新 nacos 维护的(一致性保证)的对应的开关项目的值.
   * 
   * @param entry
   * @param value
   * @param debug
  ...
  /**
   * 更新当前 nacos 维护的开关项目(该方法不会触发一致性同步),一般用于调试.
   * 
  ...
  public void update(SwitchDomain newSwitchDomain) {
    ...
    switchDomain
        .setServerStatusSynchronizationPeriodMillis(newSwitchDomain.getServerStatusSynchronizationPeriodMillis());
    switchDomain
        .setServiceStatusSynchronizationPeriodMillis(newSwitchDomain.getServiceStatusSynchronizationPeriodMillis());
