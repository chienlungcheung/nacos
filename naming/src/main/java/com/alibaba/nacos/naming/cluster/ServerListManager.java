...
public class ServerListManager {
  ...
  /**
   * 保存接收到每个服务器状态报告的时间戳(以当前节点时钟为准, 即每次更新都是取当前节点接收到报告时候的时间戳)
   */
  ...
  /**
   * 每个 nacos 节点会周期性的(当前是 15 秒)将自己的状态报告发送给集群其它 nacos 节点。 
   * <p>
   * 该方法负责解析并更新本地维护的其它 nacos 的状态信息。
   *
   * @param configInfo
   */
  public synchronized void onReceiveServerStatus(String configInfo) {
    ...
    for (String config : configs) {
      ...
      // 如果状态所属的 server 不在当前服务器列表中，则报错（集群服务器列表发生了变更，但是当前 nacos 节点还不知道）
      ...
      // 这是当前 nacos 节点第一次接收到其它 nacos 节点发送的状态报告
      ...
      for (Server s : list) {
        ...
        // 如果发送报告的 nacos 节点状态有变化, 则将其对应的新的 Server 对象加入到临时列表中
        if (serverId.equals(newServerId)) {
          if (s.isAlive() != server.isAlive() || s.getWeight() != server.getWeight()) {
            Loggers.SRV_LOG.warn("server beat out of date, current: {}, last: {}", JSON.toJSONString(server),
                JSON.toJSONString(s));
          }
          tmpServerList.add(server);
          continue;
        }
        // 否则, 将其对应的老的 Server 对象加入到临时列表中
        tmpServerList.add(s);
      }

      // 如果发送报告的 nacos 节点本次是第一次发送状态报告给当前 nacos 节点, 将其加入到临时列表
      if (!tmpServerList.contains(server)) {
        tmpServerList.add(server);
      }

      // 用新的 nacos 节点列表替换本地维护的老的列表
  ...
  /**
   * 清理本地维护的无效的 nacos 节点信息,并通知其它 nacos 节点也进行清理.
   */
  public void clean() {
    cleanInvalidServers();
  ...
  /**
   * 通知地址为 serverIP 的 nacos 节点清理其维护的无效的 nacos 节点信息.
   * 
   * @param serverIP
   */
    ...
    public void run() {
      try {
        ...
        if (!contains(NetUtils.localServer())) {
          Loggers.SRV_LOG.error("local ip is not in serverlist, ip: {}, serverlist: {}", NetUtils.localServer(),
              allServers);
          return;
        }
  ...
  private void checkDistroHeartbeat() {
    ...
    if (autoDisabledHealthCheck && curRatio > switchDomain.getDistroThreshold()
      ...
      Loggers.SRV_LOG.info(
          "[NACOS-DISTRO] distro threshold restored and " + "stable now, enable health check. current ratio: {}",
          curRatio);
